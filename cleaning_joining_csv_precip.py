...
format_date(df_precip_PIT)
format_date(df_precip_CHA)
format_date(df_precip_CLE)
format_date(df_precip_PHO)
format_date(df_precip_LV)
format_date(df_temp_PIT)
format_date(df_temp_CHA)
format_date(df_temp_CLE)
format_date(df_temp_PHO)
format_date(df_temp_LV)


# Start in 2004, end in 2017
def filter_date(df):
...
# Change name of temperature data
...
# differentiate cumulative precipitation
def diff_cumulative_precip(df):
    year=[g for n,g df.groupby(pd.Grouper(key='date', freq='Y'))]
    for y in year:
        y['precipitation']=y['precipitation'].diff()
        y['precipitation_normal']=y['precipitation_normal'].diff()
    return df
...
df_PIT=pd.merge(df_precip_PIT,df_temp_PIT, how='left', on=['date', 'city'])
df_CHA=pd.merge(df_precip_CHA,df_temp_CHA, how='left', on=['date', 'city'])
df_CLE=pd.merge(df_precip_CLE,df_temp_CLE, how='left', on=['date', 'city'])
df_PHO=pd.merge(df_precip_PHO,df_temp_PHO, how='left', on=['date', 'city'])
df_LV=pd.merge(df_precip_LV,df_temp_LV, how='left', on=['date', 'city'])
...
# Export as csv
df_weather.to_csv('C:/Users/natr/Desktop/HSLU_S2/DBM/project/py_charm/data/weather_data.csv', index=False)
